...
impl AttestationDriver<'_> {
    ...
    fn attestation(&mut self, n: NegotiationResponse) -> Result<Vec<u8>, AttestationError> {
        ...
        let Some(decryption) = resp.decryption else {
            return Err(AttestationError::PublicKeyMissing)?;
        };
        let Some(mut secret) = resp.secret else {
            return Err(AttestationError::SecretMissing);
        };

        // secret resource containing key and tmc
        ...
        // verbose_log!(info, "[AttestationDriver] Decrypted secret and tmc are:\n({:?}, {:?})", &secret, tmc);
        Ok(secret)
    }
    ...
    pub fn resource(&mut self) -> Result<Vec<u8>, AttestationError> {
        ...
        self.decrypt(&mut secret, decryption)?;
        // verbose_log!(info, "[DynamicDetectionDriver] Resource secret before return:\n{:?}", &secret);
        Ok(secret)
    }
    ...
    fn read(&mut self) -> Result<Vec<u8>, AttestationError> {
        ...
        verbose_log!(info, "[svsm->driver] read {} bytes from aproxy", &len);
    ...
    fn write(&mut self, param: impl Serialize) -> Result<(), AttestationError> {
        ...
        verbose_log!(info, "[driver->svsm] write {} bytes to aproxy", bytes.len());
